...
| File | Purpose |
|------|---------|
| `app.py` | argparse CLI layer (incl. `--bicolor`, which is normalized to `None` for `koipond`/`donut` since those modes ignore it), curses setup, a single session-wide `ColorHelper` threaded through `show_menu(ch=...)` and each `_run_*(..., ch=...)` to stop curses pair-counter growth across menu↔anim cycles, shared `_run_animation()` loop (~20fps) used by all three modes, per-mode callback factories (`_run_lamp`, `_run_donut`, `_run_pond`), layout, resize handling, menu-to-lamp/pond flow, `--duration` deadline (re)computed at each animation launch so menu navigation never eats into the budget |
...
| `lamp.py` | Core lava simulation: shape profiles (incl. rocket and fireplace), `Ball`/`Lamp` classes, metaball field computation via the unified `compute_field_cell()` (single pass over both half-blocks, distance-based cutoff, bicolor-aware in one routine), fireplace/campfire ember physics (`_update_fireplace`), Perlin noise field sampled once per cell at the midpoint and reused for both halves, pre-cached campfire scene buffers (`_forest_buf`, `_log_buf`) for the time-invariant background, unified trail buffer + decay via `_apply_trail()` (preserves the `-1` "outside glass" sentinel), half-block rendering, solid base/cap/frame rendering, binary-search profile interpolation via `_interpolate_profile()` with pre-cached y-keys, `field_to_level()` via `bisect_right` |
...
| Fireplace | Fullscreen rising embers (inverted gravity, temperature-scaled metaball field, teardrop flame shaping, procedural 3D logs and pine forest background); both forest and log scenes are deterministic from `(px, py)` and rasterized once at construct/resize into `_forest_buf` / `_log_buf` lookup tables, then read per cell instead of recomputed every frame | `SHAPES['fireplace']` + `FLOW_PARAMS['fireplace']` + `Lamp._update_fireplace()` + `Lamp._build_scene_buffers()` + `Lamp._compute_log_color()` + `Lamp._compute_forest_bg()` |
| Campfire | Same ember physics as Fireplace, always with the layered pine forest + starry sky background (also cached via `_build_scene_buffers`) | `SHAPES['campfire']` + `FLOW_PARAMS['fireplace']` + `Lamp._build_scene_buffers()` |
...
Fireplace and campfire reuse the metaball engine but swap in `FLOW_PARAMS['fireplace']` (negative gravity, no buoyancy, no wall bounce, strong flicker `random_force`, slight outward `swirl`) regardless of the user's flow pick. `Lamp.__init__` forces `flow_type = 'fireplace'` for both styles, spawning `2× num_balls` small, hot embers at the bottom in a Gaussian cluster with 8% of terminal width spread (wide enough to match the log base). `_update_fireplace()` drifts each ember upward with flicker jitter and cooling. The engine uses asymmetric teardrop shaping (squashed bottom, long wisp tail) and sine-wave "licking" motion for the tails in `compute_field_cell()` — both `fireplace` and `campfire` styles apply flame squashing via the `fireplace = self.style in ('fireplace', 'campfire')` guard. A sustained procedural flame pillar emerges from the logs to form a solid fire core; when `--bicolor` is on, the pillar's contribution splits 50/50 between the palette-A and palette-B sums so the central spine inherits whichever palette dominates among the surrounding embers (rather than being permanently locked to palette A). `Lamp._compute_log_color()` and `Lamp._compute_forest_bg()` produce 3D log shading and a layered pine tree silhouette background, but they are only called once per `(px, py)` during `Lamp.__init__` / `Lamp.resize()` — the results are stored in `_log_buf` and `_forest_buf` 2-D lookup tables and read per cell via `_get_log_color()` / `_get_forest_bg()`, so the per-frame render path is a tight list-of-list index rather than four trig-and-hash calls per cell.

### Bi-color Metaball Rendering

Each `Ball` carries a `palette_id` (0 or 1). `Lamp(..., bicolor=True)` assigns palettes round-robin so the two groups start balanced. `add_ball()` / `remove_ball()` preserve the balance by adding to the minority and removing from the majority. `Lamp.compute_field_cell(px, py_t, py_b)` is the single entry point for both mono and bicolor paths — it sums metaball contributions per palette and returns `(ft, t_pid, fb, b_pid)`, where the palette IDs are always `0` outside bicolor mode and the winning palette per half otherwise. The dominant id is then threaded into `ColorHelper.draw_cell(..., top_pid, bot_pid)` so each pixel picks the winning palette's colors. Liquid (Perlin-noise) flow has no per-ball identity, so its bicolor regions are carved by a time-varying spatial mask (`sin(t·0.5 + px·0.18 + py·0.13)`) that drifts the A/B boundary across the field — both palettes still appear, just along noise contours rather than around individual blobs.

### Motion Trails (Slow-shutter Mode)

Each `Lamp` carries a lazily-allocated `trail_buffer` — a 2D list of `[top_level, bot_level, frames_remaining, top_pid, bot_pid]` sized to `body_height × body_width`. Both `render_freestyle` and `_render_body` route through a single `_apply_trail(row, col, tl, bl, t_pid, b_pid)` helper that updates the entry and returns `(gtl, gbl, gtp, gbp, ghost)` describing what to paint. Cells currently showing lava refresh their entry to `TRAIL_LIFE` (14 frames); cells that have gone back to liquid but still have life remaining are drawn with a progressively dimmer level via `_trail_decay_level()` (lava 3 → 2 → 1 → rim over the 14-frame window) and then decremented. The helper preserves the `-1` "outside glass" sentinel end-to-end and re-asserts it from the live glass bounds when painting a ghost, so the trail never bleeds liquid color past the glass curve. `Lamp.resize()` invalidates the buffer so it reallocates at the new dimensions on the next frame.
...
Then add `'my_shape'` to `SHAPE_ORDER`. For custom cap/base profiles (like rocket), define separate profile arrays and add conditionals in `_cap_bounds_at()` and `_base_bounds_at()`. For a fullscreen style with no lamp frame (like `freestyle` / `fireplace`), add the style name to the `style in ('freestyle', 'fireplace')` check in `calculate_lamp_dims()` and `_run_lamp()` in `app.py` so it uses the freestyle render path.
